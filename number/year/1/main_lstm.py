...
jul_date = np.loadtxt(file_location+r'\data1700_2021\sn_train_jul_date{}.txt'.format(in_cycle*sun_epoch)).reshape(-1,1)
...
# model = ANN.cnn1d(x_train, output_node=output_data.shape[1], 
#         layer=config.layer, layer_size=config.layer_size, 
#         rate=config.rate, weight=config.weight)
...
#     marker='+', c='grey', label=u'观测值 (测试集)', s=30)
# plt.scatter(jul_date[num1+num2:], y_test_pre, 
#     marker='+', c='indianred', label=u'预测值 (测试集)', s=30)
# plt.xlabel(u'日期', fontproperties=fontcn, fontsize=20)         
# plt.ylabel(u'太阳黑子数', fontproperties=fontcn, fontsize=20)   
# plt.xticks(x_jul, x_tick_time, size=16)
# plt.yticks(size=16)
# plt.xlim(julian.to_jd(datetime.datetime(1950,1,1,0,0,0), fmt='jd')-initial_julian, 
...
#     marker='*', c='grey', label=u'观测值 (训练集)', s=20)
# plt.scatter(jul_date[num1:num1+num2], y_val, 
#     marker='x', c='grey', label=u'观测值 (验证集)', s=25)
# plt.scatter(jul_date[num1+num2:], y_test, 
#     marker='+', c='grey', label=u'观测值 (测试集)', s=30)
# plt.scatter(jul_date[:num1], y_train_pre,
#     marker='*', c='dodgerblue', label=u'预测值 (训练集)', s=20)    
