...
# ax.scatter(jul_date[np.where(area<=80)], Latitude[np.where(area<=80)], c='k', marker='|', s=5, label='$\leq$ 80ppm')
# ax.scatter(jul_date[np.where((area>80) & (area<600))], Latitude[np.where((area>80) & (area<600))], c='r', marker='|', s=5, label='(80ppm,600ppm)')
# ax.scatter(jul_date[np.where(area>=600)], Latitude[np.where(area>=600)], c='y', marker='|', s=5, label='$\geq$ 600ppm')
ax.scatter(jul_date[np.where(area*1e-6>0)], Latitude[np.where(area*1e-6>0)], c='k', marker='|', s=5, label='>0%')
ax.scatter(jul_date[np.where(area*1e-6>0.01)], Latitude[np.where(area*1e-6>0.01)], c='r', marker='|', s=5, label='>1%')
ax.scatter(jul_date[np.where(area*1e-6>=0.1)], Latitude[np.where(area*1e-6>=0.1)], c='y', marker='|', s=5, label='>10%')
...
ax.text(0.1, 98, '可见太阳半球中黑子面积占比(%)', fontproperties=font, fontsize=18)
...
for key, value in enumerate(x_min):
    ...
    ax.text(jul, -75, '{}'.format(11+key), fontproperties=font, fontsize=18, 
...
ax.legend(loc='upper left', prop=FontProperties(fname=r"C:\WINDOWS\Fonts\simsun.ttc", size=14), \
    ncol=1) 
ax = plt.gca()
...
plt.tight_layout()
plt.savefig(r'..\figure\fig2.png', dpi=100)
plt.savefig(r'..\figure\fig2.eps', dpi=100)
plt.show()
