...
count0 = 0
with open(file_location+r'\RGO_NOAA1874_2021\sunspot_area_lat_11{}.txt'.format(
    config.blocks), mode='w+', encoding='utf-8') as fout1:
    with open(file_location+r'\RGO_NOAA1874_2021\sunspot_area_lat_11_pre{}.txt'.format(
        config.blocks), mode='w+', encoding='utf-8') as fout2:
            ...
            for m in np.arange(1, 12+1):
                if y==config.min_year and m<5:
                    continue
                        ...
                        -relativedelta(months=config.sun_epoch*12):
                    break
                elif datetime.datetime(y,m,1,0,0,0) >= datetime.datetime(2021,6,1,0,0,0)\
                        -relativedelta(months=config.sun_epoch*12*2) and \
                    datetime.datetime(y,m,1,0,0,0) < datetime.datetime(2021,6,1,0,0,0)\
                        -relativedelta(months=config.sun_epoch*12) :
                    result = []
                    for i in np.arange(0,config.sun_epoch*12,1):
                    ...
                    fout2.write('{} {} 1 {}\n'.format(y+config.sun_epoch, m, result)) 
                else:
                    result = []
                    # count0 += 1
                    # print(count0)
                    for i in np.arange(0,config.sun_epoch*12*2,1):
                        date_begin = datetime.datetime(y,m,1,0,0,0)+relativedelta(months=i)
                        date_end = datetime.datetime(y,m,1,0,0,0)+relativedelta(months=i+1)
                        days = (date_end - date_begin).days
                        flag_year = np.logical_and(date>=date_begin, date<date_end)
...
with open(file_location+r'\RGO_NOAA1874_2021\sunspot_area_lat_11{}.txt'.format(
    ...
    f.write(content)
data = np.loadtxt(file_location+r'\RGO_NOAA1874_2021\sunspot_area_lat_11{}.txt'.format(
...
np.savetxt(file_location+r'\RGO_NOAA1874_2021\sunspot_area_lat_jul_date_11{}.txt'.format(
...
with open(file_location+r'\RGO_NOAA1874_2021\sunspot_area_lat_11_pre{}.txt'.format(
    config.blocks), 'w') as f:
    f.write(content)
data = np.loadtxt(file_location+r'\RGO_NOAA1874_2021\sunspot_area_lat_11_pre{}.txt'.format(
    config.blocks), delimiter=' ')
month = np.array(data[:, 1], dtype=np.int)
day = np.array(data[:, 2], dtype=np.int)
initial_julian = julian.to_jd(datetime.datetime(1870,1,1,0,0,0), fmt='jd')
...
for j in np.arange(config.blocks):
    for i in np.arange(len(data)): 
        ...
        t_jd = julian.to_jd(t, fmt='jd')-initial_julian
        jul_date.append(t_jd)
np.savetxt(file_location+r'\RGO_NOAA1874_2021\sunspot_area_lat_jul_date_11_pre{}.txt'.format(
    config.blocks), jul_date, fmt='%.1f', delimiter=' ')