...
def checkpoints(model_name, file_name):
	# current_time = datetime.datetime.now().strftime('%Y%m%d-%H%M%S')
	current_time = config.file_name
	model_dir = os.path.join('{}'.format(model_name))
	...
	filepath = os.path.join(model_dir, 
		'{epoch:05d}-{loss:.6f}-{rmse:.6f}-' +
		'{val_loss:.6f}-{val_rmse:.6f}.h5')
	checkpoint = [tf.keras.callbacks.ModelCheckpoint(filepath, verbose=2,  
			monitor='val_rmse', save_best_only=True, save_weights_only=False),
		tf.keras.callbacks.EarlyStopping(monitor='val_rmse', 
