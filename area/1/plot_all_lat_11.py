...
sunspot = np.loadtxt(file_location+r'\RGO_NOAA1874_2021\sunspot_area_lat_11{}.txt'.\
...
jul_date = np.loadtxt(file_location+r'\RGO_NOAA1874_2021\sunspot_area_lat_jul_date_11{}.txt'.\
    format(blocks), delimiter=' ')
...
input_data = np.array(sunspot[:,3:3+blocks*input_window*sun_epoch*12])
output_data = np.array(sunspot[:,3+blocks*input_window*sun_epoch*12:])
...
x_train = x_train.reshape(len(x_train),1,input_window*sun_epoch*12,blocks,1)
x_val = x_val.reshape(len(x_val),1,input_window*sun_epoch*12,blocks,1)
x_test = x_test.reshape(len(x_test),1,input_window*sun_epoch*12,blocks,1)
input_data = input_data.reshape(len(input_data),1,input_window*sun_epoch*12,blocks,1)
y_train = y_train.reshape(len(y_train),1,input_window*sun_epoch*12,blocks,1)
y_val = y_val.reshape(len(y_val),1,input_window*sun_epoch*12,blocks,1)
y_test = y_test.reshape(len(y_test),1,input_window*sun_epoch*12,blocks,1)
output_data = output_data.reshape(len(output_data),1,input_window*sun_epoch*12,blocks,1)
print('\n\tshape: ', x_train.shape, y_train.shape,
    x_val.shape, y_val.shape, x_test.shape, y_test.shape)

y_train = y_train.reshape(y_train.shape[0],-1)
y_train = y_scaler.inverse_transform(y_train)
# y_train = y_train.reshape(y_train.shape[0],1,input_window*sun_epoch*12,blocks)
y_train_pre = model.predict(x_train)
y_train_pre = y_train_pre.reshape(y_train_pre.shape[0],-1)
y_train_pre = y_scaler.inverse_transform(y_train_pre)
# y_train_pre = y_train_pre.reshape(y_train.shape[0],1,input_window*sun_epoch*12,blocks)
y_val = y_val.reshape(y_val.shape[0],-1)
y_val = y_scaler.inverse_transform(y_val)
# y_val = y_val.reshape(y_val.shape[0],1,input_window*sun_epoch*12,blocks)
y_val_pre = model.predict(x_val)
y_val_pre = y_val_pre.reshape(y_val_pre.shape[0],-1)
y_val_pre = y_scaler.inverse_transform(y_val_pre)
# y_val_pre = y_val_pre.reshape(y_val_pre.shape[0],1,input_window*sun_epoch*12,blocks)
y_test = y_test.reshape(y_test.shape[0],-1)
y_test = y_scaler.inverse_transform(y_test)
# y_test = y_test.reshape(y_test.shape[0],1,input_window*sun_epoch*12,blocks)
y_test_pre = model.predict(x_test)
y_test_pre = y_test_pre.reshape(y_test_pre.shape[0],-1)
y_test_pre = y_scaler.inverse_transform(y_test_pre)
# y_test_pre = y_test_pre.reshape(y_test_pre.shape[0],1,input_window*sun_epoch*12,blocks)
...
h1 = plt.scatter(jul_date[-input_window*sun_epoch*12:,:].reshape(-1,), 
    y_lat_all[-input_window*sun_epoch*12:,:].reshape(-1,), 
    c=y_all[-1:,:].reshape(-1,), marker='|', s=200, cmap='Wistia', norm=norm) 
...
plt.subplot(2,1,2)
plt.grid(True, axis='x', linestyle='--', linewidth=1.0) 
h2 = plt.scatter(jul_date[-input_window*sun_epoch*12:,:].reshape(-1,), 
    y_lat_all[-input_window*sun_epoch*12:,:].reshape(-1,), 
    c=y_all_pre[-1:,:].reshape(-1,), marker='|', s=180, cmap='Wistia', norm=norm) 
plt.axvline(x=jul_date[num1,0], c="black", ls="--", lw=2)
plt.axvline(x=jul_date[num1+num2,0], c="black", ls="--", lw=2)
