...
import config
from ANN import stateless_convlstm
...
sunspot = np.loadtxt(file_location+r'\RGO_NOAA1874_2021\sunspot_area_lat_11{}.txt'.\
    format(blocks), delimiter=' ')
print(sunspot.shape)
input_data = np.array(sunspot[:,3:3+blocks*sun_epoch*12])
output_data = np.array(sunspot[:,3+blocks*sun_epoch*12:])      
...
x_train = x_train.reshape(len(x_train),1,sun_epoch*12,blocks,1)
x_val = x_val.reshape(len(x_val),1,sun_epoch*12,blocks,1)
x_test = x_test.reshape(len(x_test),1,sun_epoch*12,blocks,1)
input_data = input_data.reshape(len(input_data),1,sun_epoch*12,blocks,1)
y_train = y_train.reshape(len(y_train),1,sun_epoch*12,blocks,1)
y_val = y_val.reshape(len(y_val),1,sun_epoch*12,blocks,1)
y_test = y_test.reshape(len(y_test),1,sun_epoch*12,blocks,1)
output_data = output_data.reshape(len(output_data),1,sun_epoch*12,blocks,1)
...
model = stateless_convlstm(x_train, output_node=output_data.shape[1], 
...
y_train = y_train.reshape(y_train.shape[0],1,sun_epoch*12,blocks)
y_train_pre = model.predict(x_train)
y_train_pre = y_scaler.inverse_transform(y_train_pre)
y_train_pre = y_train_pre.reshape(y_train.shape[0],1,sun_epoch*12,blocks)
y_val = y_val.reshape(y_val.shape[0],-1)
y_val = y_scaler.inverse_transform(y_val)
y_val = y_val.reshape(y_val.shape[0],1,sun_epoch*12,blocks)
y_val_pre = model.predict(x_val)
y_val_pre = y_scaler.inverse_transform(y_val_pre)
y_val_pre = y_val_pre.reshape(y_val_pre.shape[0],1,sun_epoch*12,blocks)
y_test = y_test.reshape(y_test.shape[0],-1)
y_test = y_scaler.inverse_transform(y_test)
y_test = y_test.reshape(y_test.shape[0],1,sun_epoch*12,blocks)
y_test_pre = model.predict(x_test)
y_test_pre = y_scaler.inverse_transform(y_test_pre)
y_test_pre = y_test_pre.reshape(y_test_pre.shape[0],1,sun_epoch*12,blocks)
